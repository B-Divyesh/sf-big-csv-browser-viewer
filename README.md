...
Deploy the contents of `dist/` as an Azure Static Web App. `public/staticwebapp.config.json` supplies immutable asset caching and security headers. DuckDB and the XLSX reader are bundled locally; the 34 MB query engine is fetched only after a user chooses a file. The initial application JavaScript is about 31 KB uncompressed.
